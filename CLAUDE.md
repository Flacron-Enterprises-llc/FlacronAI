...
- **Rule #3 — human-review gate:** ✅ RESOLVED 2026-07-19 (T-2.7) for the main flow. New reports save as **`status: 'draft'`** (not 'completed'); export of an un-reviewed report is force-watermarked "DRAFT — PENDING ADJUSTER REVIEW" + filename `_DRAFT` (`isReviewed()` in `reports.js`; legacy 'completed' treated as reviewed). New **`POST /api/reports/:id/approve`** records `reviewedBy`/`reviewedAt` and sets `status:'finalized'` → clean export. Dashboard now has an editable content textarea + Save + "Approve & Finalize" + status badges + a "Review" action to reopen drafts. Verified end-to-end (draft→_DRAFT, approve→finalized+reviewer, finalized→clean). ⚠️ Follow-up: `EnterpriseDashboard.jsx` preview still lacks the approve UI (but the backend gate applies to enterprise reports too — they export watermarked until approved via API).
