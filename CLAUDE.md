# CLAUDE.md — FlacronAI Project Master Context
...
1. **CLAUDE.md** (this file) — rules, conventions, golden constraints, project map.
...
- **AI provider + image analysis (reworked 2026-07-19, T-2.5a):** **Claude (Anthropic) is PRIMARY**, IBM **WatsonX** (`ibm/granite-3-8b-instruct`) is the text-only fallback. **OpenAI fully removed.** `backend/config/anthropic.js` wraps `@anthropic-ai/sdk`; model via `ANTHROPIC_MODEL` env (default `claude-opus-4-8`). Text = `generateWithFallback()` (Claude → watsonx) in `aiService.js`. Image analysis = **Claude vision** (base64 blocks, jpeg/png/gif/webp only — heic/heif skipped, capped at 10 images); watsonx has no vision so image analysis degrades gracefully to "unavailable" if Claude isn't configured. **Needs `ANTHROPIC_API_KEY` in prod (Render) to run** — until set, everything falls back to watsonx. ⚠️ Report prompts still demand definitive verdicts (Golden Rule #2) — provider swap did NOT change prompt language; that's still Phase-2 (T-2.5) work.
- **File storage (reworked 2026-07-19):** **Firebase Storage** (bucket `flacronai-c8dab.firebasestorage.app`, `FIREBASE_STORAGE_BUCKET`) via `firebase-admin`. `backend/config/storage.js` is now a Storage abstraction: `uploadBuffer`/`downloadBuffer`/`deleteObject(s)`/`getSignedUrl`/`tokenUrl` + object-path builders (`users/{uid}/reports|exports|logos|whitelabel/...`). **No local disk, no ephemeral-loss.** Export generators are buffer-based (`generatePDF`/`generateDOCX` return Buffers, `watermarkService` buffer-in/out, `aiService.analyzeImages` takes `{buffer,mimetype}`). Claim photos + exports are **private** (read server-side or via authenticated `GET /api/reports/:id/download`, which proxies bytes from Storage); branding logos get a stable Firebase **download-token URL** (`firebaseStorageDownloadTokens`) for the browser. The old public `/uploads` static route + `middleware/uploadAccess.js` are **removed**.
- **Email/SMS provider (reworked 2026-07-19):** Email = **AWS SES** via `@aws-sdk/client-ses` (`backend/services/emailService.js`). All 6 transactional emails are branded inline HTML built by a shared `layout()` helper (no external template store): welcome, password reset, payment failed, team invite, sales lead, email verification. Sender `SES_FROM_EMAIL` (domain `flacronenterprises.com` verified in SES, us-east-1, **production mode** — 50k/day). Degrades gracefully (logs + skips) when AWS creds absent. **Brevo fully removed** (no dep; dead `scripts/*BrevoTemplates.js` deleted). **No SMS provider.**
...
**GOLDEN RULE VIOLATIONS (fix in Phase 1/2):** — Rules #1, #2, #3, #6 all RESOLVED as of 2026-07-19 (see changelog T-1.1, T-2.5, T-2.7, T-3.10a/b). The line-level detail below is the original audit snapshot, kept for reference; a 2026-07-19 grep confirmed the fabricated Rule #1 strings (SOC 2, MongoDB, fake team/stats, "Microsoft" powered-by, "All systems operational") are gone from live pages.
- **Rule #1 — fabricated claims (site-wide) [RESOLVED — T-1.1]:**
...
- **Rule #2 — AI verdicts:** ✅ RESOLVED 2026-07-19 (T-2.5). `buildReportPrompt` + the Section-7 fallback prompt fully rewritten: report is framed as an **AI-generated DRAFT for licensed-adjuster review**, with an explicit language/scope rule block (cautious "appears/may/subject to" language; NO final determination of cause, coverage, liability, fraud, structural/mold, or final cost — these become "items for the adjuster to evaluate"). Section 7 relabelled "PRELIMINARY ESTIMATED COSTS (FOR PLANNING & REVIEW ONLY)" with non-binding framing. Auto-certification removed from both exports: `properPdfGenerator.js` + `documentGenerator.js` now show "Reviewing Adjuster Sign-Off" with an AI-draft disclaimer (the adjuster's *own* signature is their attestation; the system no longer writes "I certify…"). Live-tested: generated report hit all cautious markers, zero hard-verdict phrases. (Image-analysis prompt was already cautious.) Marketing sample copy (`Home.jsx`) already cautious; `Dashboard.jsx` sample is adjuster *input*, not AI output.
- **Rule #3 — human-review gate:** ✅ RESOLVED 2026-07-19 (T-2.7) for the main flow. New reports save as **`status: 'draft'`** (not 'completed'); export of an un-reviewed report is force-watermarked "DRAFT — PENDING ADJUSTER REVIEW" + filename `_DRAFT` (`isReviewed()` in `reports.js`; legacy 'completed' treated as reviewed). New **`POST /api/reports/:id/approve`** records `reviewedBy`/`reviewedAt` and sets `status:'finalized'` → clean export. Both `Dashboard.jsx` and `EnterpriseDashboard.jsx` now have an editable content textarea + Save + "Approve & Finalize" + status badges (Dashboard also has a "Review" action to reopen drafts). Verified end-to-end (draft→_DRAFT, approve→finalized+reviewer, finalized→clean).
...
- ~~`firestore.rules:15` hardcodes admin as `admin@flacronai.com`~~ — RESOLVED 2026-07-18: `isAdmin()` now points at the real account `admin@flacronenterprises.com` (redeploy `firestore.rules` to Firebase if not already done).
- ~~`reports.js` returns `err.stack` to clients~~ — FIXED 2026-07-19 (T-3.12): export error no longer leaks stack/internals (prod → generic message). Global error handler already prod-safe; CORS allow-all is dev-only (prod restricts to `allowedOrigins`; secure default when `NODE_ENV` unset).
- ~~Custom JWTs (7-day) can't be revoked on logout~~ — RESOLVED 2026-07-22 (T-3.10d): `tokenVersion` claim checked against the user doc on every custom-JWT request; logout/password-change bump it, invalidating outstanding tokens immediately.
- `/admin` and `/admin-tier-update` frontend routes gated only client-side (`AdminDashboard.jsx:390-396` email check via `VITE_ADMIN_EMAIL`); real protection depends on backend `requireAdmin` — verify each admin endpoint has it. Admin access is single-email, not role-based (no RBAC) — fine for one admin, would need real roles before adding staff/support accounts.
- ~~Upload mimetype checks trust the client-supplied type~~ — RESOLVED 2026-07-21 (T-3.10c): magic-byte signature validation (`utils/imageValidation.js`) rejects files whose real bytes don't match an allowed image type.
- **T-3.10 (security hardening) is now DONE** except malware/virus scanning and automated data-retention/expiry, both deliberately deferred per an explicit 2026-07-22 client decision (see PROGRESS.md). Session revocation, auth rate limiting, an audit trail (`auditLogs` collection + admin viewer), new-device login alerts, self-service account deletion, and opt-in TOTP MFA (`speakeasy`/`qrcode`, Settings > Security) all shipped and were live-QA'd against the real Firebase project. Backup/recovery (Firestore PITR, Storage versioning) is a GCP console/billing decision, not code — flagged for the client, not actioned.
- Local backend `.env` is still missing `FIREBASE_API_KEY` (placeholder never added, confirmed again 2026-07-22) — blocks local testing of `/api/auth/login`, MFA login-verify, and self-service account deletion's password re-check (all three need it; each returns `CONFIG_ERROR` without it). **Verify the real key is set in Render** for these prod code paths to work.
