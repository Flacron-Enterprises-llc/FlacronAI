...
const generatePDF = async (report, options = {}) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      // page, navy title, thin orange rule, plain label/value list)
      // ══════════════════════════════════════════════════════════════════════
      ...
      if (logoBuffer) {
        try { doc.image(logoBuffer, cx - 50, coverY, { width: 100, fit: [100, 50] }); coverY += 62; } catch {}
      }
      ...
      doc.fontSize(10).fillColor(accentHex).font('Helvetica-Bold')
        .text(
          ...
          margin, coverY, { width: contentWidth, align: 'center', characterSpacing: 2 }
        );
      coverY += 26;

      doc.fontSize(26).fillColor(NAVY).font('Helvetica-Bold')
        .text('INSURANCE INSPECTION REPORT', margin, coverY, { width: contentWidth, align: 'center' });
      coverY = doc.y + 8;
      ...
      if ((report.status || 'draft') !== 'finalized') {
        doc.fontSize(11).fillColor(accentHex).font('Helvetica-Bold')
          .text('DRAFT — PENDING ADJUSTER REVIEW', margin, coverY, { width: contentWidth, align: 'center', characterSpacing: 0.5 });
        coverY = doc.y + 22;
      } else {
        coverY += 10;
      }
      ...
      doc.fontSize(8).fillColor('#94a3b8').font('Helvetica-Oblique')
        .text(
          ...
          margin, coverY, { width: contentWidth, align: 'center' }
        );
      ...
      for (const line of lines) {
        ...
        // Bullet — within Report Header / Insured Information specifically,
        ...
        if (trimmedLine.startsWith('- ') || trimmedLine.startsWith('* ')) {
          ...
          const isFieldListSection = /report header|insured information/i.test(currentSectionTitle);
          const kv = isFieldListSection ? raw.match(/^([^:]{2,60}):\s*(.*)$/) : null;
          if (kv) {
            const [, label, value] = kv;
            doc.fontSize(10).fillColor(NAVY).font('Helvetica-Bold')
              .text(stripMd(label) + ':  ', margin + 2, doc.y + 3, { continued: true, width: contentWidth - 2 });
      ...
      doc.fontSize(18).fillColor(NAVY).font('Helvetica-Bold').text('Reviewing Adjuster Sign-Off', margin, 72);
      ...
      for (let i = 0; i < range.count; i++) {
        ...
        const footerLeft = reportFooter || (hideFlacronBranding ? companyName : 'FlacronAI — Insurance Inspection Reports');
