...
- Clarified the test-environment guidance: there is no hosted sandbox and no separate test-key type; every API key is a live credential. To test safely, run a non-production deployment with isolated Firebase, storage, report-generation provider, and Stripe test-mode credentials.
