...
export default function Dashboard() {
  ...
  // Bug fix: "Open"/"Review & edit" buttons on My Reports and the Dashboard
  // recent-reports table used to pass the row object straight from the list
  // (`GET /api/reports`) into the editor. That endpoint intentionally
  // truncates `content` to 300 chars to keep list payloads small, so the
  // Review & Edit panel below rendered only a fragment -- missing/blank/
  // mis-titled sections -- while the PDF preview stayed complete because
  // `autoPreviewPDF` regenerates it server-side from the full stored report,
  // never touching this truncated client copy. Always fetch the full report
  // (same call the `?openReport=` deep-link already uses) before opening it
  // in the editor.
  const openReportForEditing = async (r) => {
    if (r.status === 'processing') {
      setAnalysisData(null);
      setAnalysisReportId(r.id);
      setActiveView('analysis');
    ...
    try {
      const res = await reportsAPI.getOne(r.id);
      const fullReport = res.data?.report || res.data;
      setGeneratedReport(fullReport);
      setPdfPreviewUrl(null);
      setActiveView('generate');
      autoPreviewPDF(fullReport);
    } catch {
      toast.error('That report could not be opened.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-bg flex flex-col">
      ...
      <div className="flex flex-1 pt-16 min-h-0">
        <DashboardSidebar activeId={activeView} onSelectView={setActiveView} />
                            ...
                            {recentLoading ? (
                              ...
                              <tr key={r.id} className="border-b border-gray-200 hover:bg-gray-100 transition-colors cursor-pointer"
                                ...
                                <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                                  <button
                                    onClick={() => openReportForEditing(r)}
                            ...
                            <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                              <div className="flex items-center gap-1">
                                {r.status === 'processing' ? (
                                  ...
                                  <>
                                    <button onClick={() => openReportForEditing(r)}
